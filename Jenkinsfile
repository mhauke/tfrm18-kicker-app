...
node {
  ...
  try {
    ...
    stage ("Create Test instance"){
      ...
      sh "sudo docker run -d --name nginx_${env.BUILD_NUMBER} -v tfrm18kickerapp_vol_html_root_${env.BUILD_NUMBER}:/code -v tfrm18kickerapp_vol_site_config_${env.BUILD_NUMBER}:/etc/nginx/conf.d/ --net=tfrm18kickerapp_default -p 80 nginx:latest"  
    }
    ...
    stage ("Automated Test Cases"){
      ...
      sh "sudo curl http://${env.MH_DOCKER_HOST_IP}:${TEST_PORT}/v1/ping -o curl.out"
      sh "sudo cat curl.out"
      sh "sudo awk \'/true/{f=1} END{exit!f}\' curl.out"
